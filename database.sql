...
-- AUDITORIA DE ALTERAÇÕES ADMINISTRATIVAS EM PERFIS
...
create index if not exists idx_admin_profile_audit_target_created on public.admin_profile_audit(target_id, created_at desc);
create index if not exists idx_admin_profile_audit_actor_created on public.admin_profile_audit(actor_id, created_at desc);
...
-- Alterações administrativas são feitas apenas pela Edge Function após confirmar a senha.
...
create policy "admin_profile_audit_admin_read" on public.admin_profile_audit for select to authenticated using ((select public.is_admin()));
