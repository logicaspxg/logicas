(() => {
  ...
  async function loadStories(){
    ...
    $('storyList').innerHTML=stories.length?stories.map(s=>`<article class="admin-post-item story-item"><div class="admin-post-icon ${s.status==='new'?'cover-yellow':'cover-dark'}">💡</div><div class="admin-post-copy"><div class="post-meta"><span>${esc(storyStatusLabel[s.status]||s.status)}</span><time>${new Date(s.created_at).toLocaleString('pt-BR')}</time></div><h3>${esc(s.subject)}</h3><p class="story-message" id="story-message-${s.id}"><strong>${esc(s.profiles?.username||'Usuário removido')}:</strong> ${esc(s.story)}</p><button class="story-toggle" type="button" data-story-toggle aria-expanded="false" aria-controls="story-message-${s.id}" hidden>Ver mais</button></div><div class="admin-item-actions">${s.status==='new'?`<button class="mini-btn" data-story-status="read" data-story-id="${s.id}">Marcar lida</button>`:''}${s.status!=='used'?`<button class="mini-btn" data-story-status="used" data-story-id="${s.id}">Aproveitar</button>`:''}${s.status!=='archived'?`<button class="mini-btn danger" data-story-status="archived" data-story-id="${s.id}">Arquivar</button>`:''}</div></article>`).join(''):'<div class="empty-state" style="display:block">Nenhuma história recebida.</div>';
    requestAnimationFrame(()=>document.querySelectorAll('.story-message').forEach(message=>{const toggle=message.nextElementSibling;if(toggle?.matches('[data-story-toggle]'))toggle.hidden=message.scrollHeight<=message.clientHeight+1;}));
  }
  ...
  $('storyList').addEventListener('click',async e=>{const toggle=e.target.closest('[data-story-toggle]');if(toggle){const message=toggle.previousElementSibling,expanded=toggle.getAttribute('aria-expanded')==='true';message.classList.toggle('expanded',!expanded);toggle.setAttribute('aria-expanded',String(!expanded));toggle.textContent=expanded?'Ver mais':'Ver menos';return;}const b=e.target.closest('[data-story-status]');if(!b)return;b.disabled=true;const {error}=await db.from('story_submissions').update({status:b.dataset.storyStatus,reviewed_at:new Date().toISOString(),reviewed_by:currentAdminId}).eq('id',b.dataset.storyId);if(error){b.disabled=false;return showToast(error.message||'Não foi possível atualizar a história.');}showToast('História atualizada.');await loadStories();});
